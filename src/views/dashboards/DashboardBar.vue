  ...
  <LayoutDefault v-if="dashboard" width="lg" class="min-h-screen flex flex-col px-4">
    ...
    <header class="pt-3 pb-4 flex items-center justify-between">
      <div class="flex items-center gap-3 grow">
        ...
        <svg v-if="isLoading || isReporting || isUpdating" class="inline w-6 h-6 ml-2 text-violet-600 animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div v-if="!analysisStore.isLoading && dashboard[activeAnalysisType]">
          ...
          <nav class="flex justify-between border-b mb-6">
            <div class="flex space-x-6">
              <button @click="activeAnalysisType = 'median_analysis'" :class="[activeAnalysisType == 'median_analysis' ? 'border-violet-500 text-violet-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700', 'flex items-center whitespace-nowrap border-b-2 pt-5 pb-3 text-sm font-medium']">Average</button>
              <button @click="activeAnalysisType = 'max_analysis'" :class="[activeAnalysisType == 'max_analysis' ? 'border-violet-500 text-violet-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700', 'flex items-center whitespace-nowrap border-b-2 pt-5 pb-3 text-sm font-medium']">Maximum</button>
            </div>
          </nav>
    ...
    >
      ...
      <div @click="toggleAddFunnelModal()" class="flex items-center justify-center border border-violet-400 border-dashed rounded-xl py-8 px-2 cursor-pointer hover:bg-violet-50">
        <h2 class="text-lg font-medium text-violet-600">Add a funnel</h2>
      </div>
    </VueDraggableNext>
