  ...
  <template v-if="recommendationStore.isPushToWordPressPanelOpen">
      ...
      <div class="fixed right-0 top-0 h-full overflow-y-auto w-[60em] p-6 transform rounded-l-3xl bg-white shadow-2xl ring-1 ring-gray-900/5 z-50">
        ...
        <div class="flex items-center gap-2 mt-6">
          <AppButton v-if="!isLoading && blocks.length !== 0" @click="createPageInWordPress()" variant="primary" size="base" class="flex items-center gap-2">
          ...
          <AppButton v-if="!isLoading" @click="predictCMSBlocks()" variant="secondary" size="base" class="flex items-center gap-2">
            {{ blocks.length !== 0 ? 'Re-predict' : 'Predict' }} CMS Blocks
          </AppButton>
        </div>
