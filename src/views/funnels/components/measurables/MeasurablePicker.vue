            ...
            <div class="divide-y divide-gray-200">
                ...
                <div class="relative">
                    <div class="flex justify-between items-center">
                        ...
                        <div v-if="report && report.rows" class="text-gray-400 text-sm mr-4">Showing {{ report.rows.length }} of {{ report.rowCount }} results</div>
                    </div>
                </div>
                    ...
                    <div class="h-[50vh] w-[40vw] flex-none flex-col divide-y divide-gray-100 overflow-y-auto sm:flex">
                        <table v-if="!loading && report.rows" class="min-w-full max-w-full divide-y divide-gray-300">
                            ...
                            <tr v-for="row in filteredReportRows" class="divide-x divide-gray-200 cursor-pointer hover:bg-gray-50">
                                ...
                                <td v-for="value in row.dimensionValues" @click="metric.measurable = value.value" class="py-3 px-4 text-sm text-gray-500 break-all">{{ value.value }}</td>
