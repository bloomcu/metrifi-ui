    ...
    <div class="divide-y divide-gray-200">
      ...
      <!-- <div class="relative">
          <div class="flex justify-between items-center">
              <nav> -->
                  ...
                  <!-- <button @click="setMeasurablePickerTab('pageUsers')" :class="measurablePickerTab == 'pageUsers' ? 'border-indigo-500 text-indigo-600' : 'border-transparent hover:border-gray-300 text-gray-500 hover:text-gray-700'" class="group inline-flex items-center border-b-2 py-4 px-4 text-sm font-medium">
                      <EyeIcon :class="measurablePickerTab == 'pageUsers' ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-500'" class="-ml-0.5 mr-2 h-5 w-5" aria-hidden="true" />
                      <span>Page users</span>
                  </button>
                  <button @click="setMeasurablePickerTab('pagePlusQueryStringUsers')" :class="measurablePickerTab == 'pagePlusQueryStringUsers' ? 'border-indigo-500 text-indigo-600' : 'border-transparent hover:border-gray-300 text-gray-500 hover:text-gray-700'" class="group inline-flex items-center border-b-2 py-4 px-4 text-sm font-medium">
                      <EyeIcon :class="measurablePickerTab == 'pagePlusQueryStringUsers' ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-500'" class="-ml-0.5 mr-2 h-5 w-5" aria-hidden="true" />
                      <span>Page + query string users</span>
                  </button>
                  <button @click="setMeasurablePickerTab('outboundLinkUsers')" :class="measurablePickerTab == 'outboundLinkUsers' ? 'border-indigo-500 text-indigo-600' : 'border-transparent hover:border-gray-300 text-gray-500 hover:text-gray-700'" class="group inline-flex items-center border-b-2 py-4 px-4 text-sm font-medium">
                      <EyeIcon :class="measurablePickerTab == 'outboundLinkUsers' ? 'text-indigo-600' : 'text-gray-400 group-hover:text-gray-500'" class="-ml-0.5 mr-2 h-5 w-5" aria-hidden="true" />
                      <span>Outbound link users</span>
                  </button>
              </nav> -->
      ...
      <div class="flex transform-gpu divide-x divide-gray-100" as="div">
          ...
          <div class="h-[50vh] w-[40vw] flex-none flex-col divide-y divide-gray-100 overflow-y-auto sm:flex">
            ...
            <table v-if="!isReportLoading && reports[activeReport]" class="min-w-full max-w-full divide-y divide-gray-300">
                <thead>
                <tr class="divide-x divide-gray-200">
                    <th v-for="header in reports[activeReport].dimensionHeaders" scope="col" class="py-3 px-4 text-left">
                    ...
                    <th v-for="header in reports[activeReport].metricHeaders" scope="col" class="py-3 px-4 text-left">
                    <div class="text-sm font-semibold text-gray-900">{{ dictionary[header.name].displayName ?? header.name }}</div>
                    <div class="mt-0.5 text-xs italic font-normal text-gray-400">{{ header.name }}</div>
                    </th>
                </tr>
...
const selectedTab = ref(tabs.value[props.activeReport])
...
const filteredReportRows = computed(() => {
  return reports.value[props.activeReport].rows.filter(row => {
    if (JSON.stringify(row.dimensionValues).includes(searchInput.value)) {
      return row
    }
  })
...
onMounted(() => {
  ...
  if (reports.value[props.activeReport]) return
