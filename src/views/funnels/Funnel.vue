  ...
  <LayoutDefault v-if="funnelStore.funnel" width="full" class="min-h-screen flex flex-col">
    ...
    <header class="border-b flex items-center gap-3 justify-between py-3 px-4 sm:px-6 lg:px-8">
      <div class="flex items-center gap-2 grow">
        ...
        <svg v-if="funnelStore.isLoading || isUpdating" class="inline w-6 h-6 ml-2 text-violet-600 animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <nav class="min-w-[24rem] max-w-[24rem] border-r">
        ...
        <div class="flex items-center justify-between border-b py-3 pr-4 pl-4 sm:pl-6 lg:pl-8">
          ...
          <div class="flex gap-2">
            <!-- Generate with AI -->
            <!-- <button @click.stop="toggleModal()" type="button" class="group inline-flex items-center gap-1 rounded-md py-1 px-2 text-sm text-violet-600 bg-violet-50 hover:bg-violet-100 active:translate-y-px">
          ...
          >
            ...
            >
              <div class="flex items-center gap-x-2">
                ...
                <span class="inline-flex items-center rounded-md bg-violet-100 px-2 py-1 text-xs font-medium text-violet-600">{{ index + 1 }}</span>
                <p>{{ step.name }}</p>
              </div>
        ...
        <div v-else class="py-12 text-center">
          <QueueListIcon class="mx-auto mb-2 h-6 w-6 text-violet-600" aria-hidden="true" />
      ...
      <div class="flex flex-col mx-auto w-full max-w-8xl overflow-hidden px-10 py-4">
        ...
        <div v-if="isGeneratingSteps" class="rounded-md bg-violet-50 p-4 mb-4">
          <div class="flex">
            ...
            <div class="ml-3">
              <p class="text-sm font-medium text-violet-600">Generating funnel steps, please wait.</p>
            </div>
          </div>
        </div>
              ...
              <div v-if="message.show" class="border-t text-sm p-4">
                ...
                <ol class="list-decimal list-inside space-y-2 mb-5">
                  <li v-for="link in message.json['links']">
                    <a :href="link" class="hover:text-violet-500 underline" target="_blank">{{ link }}</a>
                  </li>
                </ol>
